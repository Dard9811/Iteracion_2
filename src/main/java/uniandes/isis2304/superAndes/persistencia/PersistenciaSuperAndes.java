...
public class PersistenciaSuperAndes 
{
	...
	/**
	 * Atributo para el acceso a la tabla VENTA_EMPRESA de la base de datos
	 */
	private SQLVentaEmpresa sqlVentaEmpresa;

	/**
	 * Atributo para el acceso a la tabla VENTA_PERSONA_NAT de la base de datos
	 */
	...
	/**
	 * Atributo para el acceso a la tabla VENTA_SUCURSAL de la base de datos
	 */
	...
	private void crearClasesSQL ()
	{
		...
		sqlVentaSucursal = new SQLVentaSucursal(this);
	}
	...
	public String darTablaVentaSucursal()
	{
	...
	public Bodega adicionarBodega(long espacio, long idSucursal, long cantidadMin) 
	{
		...
		try
		{
			tx.begin();
			long id = nextval();
			long tuplasInsertadas = sqlBodega.adicionarBodega (pm, id, espacio, idSucursal, cantidadMin);
			tx.commit();
	...
	public Producto adicionarProducto(String codigo_barras, String nombre, String marca, String categoria, long precio_unitario, long precio_medida, String presentacion, long cantidad_presentacion, String unidad_medida, String especificacion_empacado, long idProveedor) 
	{
		...
		{
			tx.begin();
			long tuplasInsertadas = sqlProducto.adicionarProducto(pmf.getPersistenceManager(), codigo_barras, nombre, marca, categoria, precio_unitario, precio_medida, presentacion, cantidad_presentacion, unidad_medida, especificacion_empacado, idProveedor);
			tx.commit();

			log.trace ("Inserción de producto: " + codigo_barras + ": " + tuplasInsertadas + " tuplas insertadas");

			return new Producto (codigo_barras, nombre, marca, categoria, precio_unitario, precio_medida, presentacion, cantidad_presentacion, unidad_medida, especificacion_empacado, idProveedor);
		}
		catch (Exception e)
		{
			//	        	e.prlongStackTrace();
	...
	/* ****************************************************************
	 * 			Métodos para manejar las VENTAS - SUCURSALES
	 *****************************************************************/
	 ...
	 * @return El objeto Bebida adicionado. null si ocurre alguna Excepción
	 */
	public VentaSucursal adicionarVentaSucursal (long idVenta, long idSucursal) 
	{
		PersistenceManager pm = pmf.getPersistenceManager();
		Transaction tx=pm.currentTransaction();
		try
		{
			tx.begin(); 
			long tuplasInsertadas = sqlVentaSucursal.adicionarVentaSucursal(pm, idVenta, idSucursal);
			tx.commit();

			log.trace ("Inserción ventaSucursal: " + idVenta + ": " + tuplasInsertadas + " tuplas insertadas");
			return new VentaSucursal(idVenta, idSucursal);
		}
		catch (Exception e)
		{
			//        	e.prlongStackTrace();
	 ...
	 * @return El número de tuplas eliminadas. -1 si ocurre alguna Excepción
	 */
	public long eliminarVentaSucursalPorIdVenta (long idVenta) 
	{
		PersistenceManager pm = pmf.getPersistenceManager();
		Transaction tx=pm.currentTransaction();
		try
		{
			tx.begin();
			long resp = sqlVentaSucursal.eliminarVentaSucursalPorIdVenta(pm, idVenta);
			tx.commit();
	 ...
	 * @return La lista de objetos Bebida, construidos con base en las tuplas de la tabla PROMOCION
	 */
	public List<VentaSucursal> darVentaSucursales ()
	{
